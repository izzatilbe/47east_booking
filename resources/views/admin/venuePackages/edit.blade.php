...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.venuePackage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.venue-packages.update", [$venuePackage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('accoms') ? 'is-invalid' : '' }}" name="accoms[]" id="accoms" multiple required>
                    @foreach($accoms as $id => $accom)
                        <option value="{{ $id }}" {{ (in_array($id, old('accoms', [])) || $venuePackage->accoms->contains($id)) ? 'selected' : '' }}>{{ $accom }}</option>
                    @endforeach
                </select>
                @if($errors->has(''))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('venues') ? 'is-invalid' : '' }}" name="venues[]" id="venues" multiple required>
                    @foreach($venues as $id => $venue)
                        <option value="{{ $id }}" {{ (in_array($id, old('venues', [])) || $venuePackage->venues->contains($id)) ? 'selected' : '' }}>{{ $venue }}</option>
                    @endforeach
                </select>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_package_charge') ? 'is-invalid' : '' }}" type="number" name="total_package_charge" id="total_package_charge" value="{{ old('total_package_charge', $venuePackage->total_package_charge) }}" step="0.01" required>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
