...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.dormitoryBooking.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.dormitory-bookings.update", [$dormitoryBooking->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('accom') ? 'is-invalid' : '' }}" name="accom_id" id="accom_id" required>
                    @foreach($accoms as $id => $accom)
                        <option value="{{ $id }}" {{ ($dormitoryBooking->accom ? $dormitoryBooking->accom->id : old('accom_id')) == $id ? 'selected' : '' }}>{{ $accom }}</option>
                    @endforeach
                </select>
                @if($errors->has(''))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('booked_by') ? 'is-invalid' : '' }}" name="booked_by_id" id="booked_by_id">
                    @foreach($booked_bies as $id => $booked_by)
                        <option value="{{ $id }}" {{ ($dormitoryBooking->booked_by ? $dormitoryBooking->booked_by->id : old('booked_by_id')) == $id ? 'selected' : '' }}>{{ $booked_by }}</option>
                    @endforeach
                </select>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
            ...
            <div class="form-group">
                ...
                @foreach(App\DormitoryBooking::DURATION_MONTHS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('duration_months') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="duration_months_{{ $key }}" name="duration_months" value="{{ $key }}" {{ old('duration_months', $dormitoryBooking->duration_months) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('move_in') ? 'is-invalid' : '' }}" type="text" name="move_in" id="move_in" value="{{ old('move_in', $dormitoryBooking->move_in) }}" required>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('room_charge') ? 'is-invalid' : '' }}" type="number" name="room_charge" id="room_charge" value="{{ old('room_charge', $dormitoryBooking->room_charge) }}" step="0.01" required>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('misc_charge') ? 'is-invalid' : '' }}" type="number" name="misc_charge" id="misc_charge" value="{{ old('misc_charge', $dormitoryBooking->misc_charge) }}" step="0.01">
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_charge') ? 'is-invalid' : '' }}" type="number" name="total_charge" id="total_charge" value="{{ old('total_charge', $dormitoryBooking->total_charge) }}" step="0.01" required>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                @foreach(App\DormitoryBooking::BOOKING_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('booking_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="booking_status_{{ $key }}" name="booking_status" value="{{ $key }}" {{ old('booking_status', $dormitoryBooking->booking_status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                @foreach(App\DormitoryBooking::PAYMENT_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('payment_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="payment_type_{{ $key }}" name="payment_type" value="{{ $key }}" {{ old('payment_type', $dormitoryBooking->payment_type) === (string) $key ? 'checked' : '' }}>
