...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.venueBooking.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.venue-bookings.update", [$venueBooking->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('booked_by') ? 'is-invalid' : '' }}" name="booked_by_id" id="booked_by_id">
                    @foreach($booked_bies as $id => $booked_by)
                        <option value="{{ $id }}" {{ ($venueBooking->booked_by ? $venueBooking->booked_by->id : old('booked_by_id')) == $id ? 'selected' : '' }}>{{ $booked_by }}</option>
                    @endforeach
                </select>
                @if($errors->has(''))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('datetime_start') ? 'is-invalid' : '' }}" type="text" name="datetime_start" id="datetime_start" value="{{ old('datetime_start', $venueBooking->datetime_start) }}" required>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('datetime_end') ? 'is-invalid' : '' }}" type="text" name="datetime_end" id="datetime_end" value="{{ old('datetime_end', $venueBooking->datetime_end) }}" required>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" type="text" name="duration" id="duration" value="{{ old('duration', $venueBooking->duration) }}" required>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('room_charge') ? 'is-invalid' : '' }}" type="number" name="room_charge" id="room_charge" value="{{ old('room_charge', $venueBooking->room_charge) }}" step="0.01" required>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('misc_charge') ? 'is-invalid' : '' }}" type="number" name="misc_charge" id="misc_charge" value="{{ old('misc_charge', $venueBooking->misc_charge) }}" step="0.01">
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_charge') ? 'is-invalid' : '' }}" type="number" name="total_charge" id="total_charge" value="{{ old('total_charge', $venueBooking->total_charge) }}" step="0.01" required>
                @if($errors->has(''))
                    <span class="text-danger">{{ $errors->first('') }}</span>
                @endif
            ...
            <div class="form-group">
                ...
                @foreach(App\VenueBooking::PAYMENT_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('payment_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="payment_type_{{ $key }}" name="payment_type" value="{{ $key }}" {{ old('payment_type', $venueBooking->payment_type) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\VenueBooking::BOOKING_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('booking_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="booking_status_{{ $key }}" name="booking_status" value="{{ $key }}" {{ old('booking_status', $venueBooking->booking_status) === (string) $key ? 'checked' : '' }} required>
