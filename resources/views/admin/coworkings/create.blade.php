...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.coworking.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.coworkings.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                @foreach(App\Coworking::PASS_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('pass_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="pass_type_{{ $key }}" name="pass_type" value="{{ $key }}" {{ old('pass_type', '') === (string) $key ? 'checked' : '' }} required>
